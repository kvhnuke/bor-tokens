...
func DoCallWithState(ctx context.Context, b Backend, args CallArgs, prevState *PreviousState, blockNrOrHash rpc.BlockNumberOrHash, timeout time.Duration, globalGasCap uint64) (*core.ExecutionResult, *PreviousState, error) {
	...
	evm, vmError, err := b.GetEVM(ctx, msg, prevState.state, prevState.header, &vm.Config{})
	if err != nil {
		return nil, nil, err
	}
...
func DoEstimateGasWithState(ctx context.Context, b Backend, args CallArgs, prevState *PreviousState, blockNrOrHash rpc.BlockNumberOrHash, gasCap uint64) (hexutil.Uint64, *PreviousState, error) {
